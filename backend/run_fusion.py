...
# Make paths work whether run from backend/ or workspace root
...
def main():
    ...
    # Send alerts for detected attacks (best-effort). We create a minimal PacketWithPrediction
    # Packet metadata may not be available in fusion output; fill with placeholders when missing.
    from datetime import datetime as _dt

    sent = 0
    for idx, row in decision_df[decision_df['attack'] == True].iterrows():
        # prepare packet metadata (placeholders if missing)
        pkt_id = int(idx)
        ...
        # timestamp from fusion is ISO string; convert to epoch float if possible
        ...
        # Use fusion_score as final confidence where available
        conf = float(row.get('fusion_score', row.get('xgb_confidence', 0.0)))

        # Attack type
        attack_type = row.get('attack_type', 'Unknown')
        
        # Severity from decision engine
        severity = row.get('severity', 'Low')

        # Build PacketWithPrediction
        try:
            ...
            # Send alert with severity from decision engine (use_email=False for stdout/file)
